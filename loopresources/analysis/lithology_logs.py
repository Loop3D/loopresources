...
class LithologyLogs:
    ...
    def apply_smoothing_filter(
        ...
        for hole_id in table[DhConfig.holeid].unique():
            ...
            smoothed_midpoints = uniform_filter1d(midpoints.values, size=window_size, mode='nearest')
            
            # Calculate smoothed interval thicknesses
            thicknesses = hole_data[DhConfig.sample_to] - hole_data[DhConfig.sample_from]
            smoothed_thicknesses = uniform_filter1d(thicknesses.values, size=window_size, mode='nearest')
    ...
    def identify_lithological_pairs(
        self, 
        store_as: Optional[str] = None
    ) -> pd.DataFrame:
        """
