...
class DrillholeDatabase:
    ...
    def filter(self,
               ...
               expr: Optional[Union[str, Callable]] = None) -> 'DrillholeDatabase':
        """
        ...
        if depth_range is not None:
            ...
            depth_mask = (
                (filtered_survey[DhConfig.depth] >= min_depth) &
        ...
        for name, table in self.intervals.items():
            ...
            if depth_range is not None and not filtered_table.empty:
                min_depth, max_depth = depth_range
                depth_mask = (
                    (filtered_table[DhConfig.sample_from] <= max_depth) &
                    (filtered_table[DhConfig.sample_to] >= min_depth)
                )
                filtered_table = filtered_table[depth_mask].copy()
                
                # Clip interval boundaries
                filtered_table[DhConfig.sample_from] = filtered_table[DhConfig.sample_from].clip(lower=min_depth)
                filtered_table[DhConfig.sample_to] = filtered_table[DhConfig.sample_to].clip(upper=max_depth)
        ...
        for name, table in self.points.items():
            ...
            if depth_range is not None and not filtered_table.empty:
                min_depth, max_depth = depth_range
                depth_mask = (
                    (filtered_table[DhConfig.depth] >= min_depth) &
    ...
    def validate(self) -> bool:
        ...
        for name, table in self.intervals.items():
            ...
            missing_holes = interval_holes - collar_holes
            if missing_holes:
                raise ValueError(f"Interval table '{name}' has holes not in collar: {missing_holes}")
            ...
            for hole_id in interval_holes:
                ...
                if max_to > total_depth:
                    raise ValueError(f"Interval in table '{name}' for hole '{hole_id}' exceeds total depth")
        ...
        for name, table in self.points.items():
            ...
            for hole_id in point_holes:
                ...
                if max_depth > total_depth:
                    raise ValueError(f"Point in table '{name}' for hole '{hole_id}' exceeds total depth")
