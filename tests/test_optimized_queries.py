...
class TestOptimizedQueries:
    ...
    def sample_survey(self):
        ...
        return pd.DataFrame({
            DhConfig.holeid: ['DH001', 'DH001', 'DH002', 'DH002', 'DH003', 'DH003', 'DH004', 'DH005'],
    ...
    def test_drillhole_uses_optimized_methods_file(self, sample_collar, sample_survey, temp_db_path):
    ...
    def test_query_only_fetches_specific_hole(self, sample_collar, sample_survey, temp_db_path):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(DISTINCT HOLEID) FROM collar WHERE project_id = (SELECT id FROM projects WHERE name = 'test')")
