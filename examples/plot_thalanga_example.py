...
print(f"Depth: {collar[DhConfig.total_depth].min():.1f} to {collar[DhConfig.total_depth].max():.1f}")
...
# -------
print(f"\nSummary:")
print(f"Successfully loaded Thalanga drillhole data into loopresources DrillholeDatabase")
print(f"The database contains {len(db.list_holes())} drillholes")
print(f"You can now access individual holes using: hole = db['HOLE_ID']")
print(f"Next steps would be to load assay or geological data using add_interval_table() or add_point_table() methods")