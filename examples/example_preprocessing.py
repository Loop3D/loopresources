...
assay = pd.DataFrame({
    ...
    'CU_PPM': [500.0, 'invalid', -100.0, 0.0, 1200.0, 800.0, 650.0],  # Mixed types, negative, zero
...
result_db = (db
    .validate_numerical_columns('assay', ['CU_PPM', 'AU_PPM', 'AG_PPM'], allow_negative=False)
...
print("\nFinal assay data:")
print(result_db.points['assay'][['HOLEID', 'DEPTH', 'CU_PPM', 'AU_PPM', 'AG_PPM']].to_string(index=False))
...
print(f"After preprocessing: {len(result_db.points['assay'])} records across {len(result_db.list_holes())} holes")
